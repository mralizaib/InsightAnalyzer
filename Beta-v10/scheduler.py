...
from models import AlertConfig, ReportConfig, SystemConfig, db, RemediationPolicy, RemediationExecution
from email_alerts import EmailAlerts
from report_generator import ReportGenerator
from n8n_integration import get_n8n_client
...
def trigger_remediation_workflows(alert_config):
    """Trigger n8n remediation workflows for high/critical alerts"""
    try:
        # Get all enabled remediation policies matching this alert config
        policies = RemediationPolicy.query.filter_by(
            user_id=alert_config.user_id,
            enabled=True,
            auto_execute=True  # Only auto-execute policies
        ).all()
        
        if not policies:
            logger.debug(f"No remediation policies found for alert config {alert_config.id}")
            return
        
        # Get the n8n client
        client = get_n8n_client()
        
        # Check each policy's severity criteria
        alert_levels = alert_config.get_alert_levels()
        for policy in policies:
            policy_levels = policy.get_severity_levels()
            
            # Check if policy matches this alert's severity
            matches = any(level in policy_levels for level in alert_levels)
            
            if not matches:
                logger.debug(f"Policy {policy.id} does not match alert levels {alert_levels}")
                continue
            
            logger.info(f"🔧 Triggering remediation policy {policy.name} (ID: {policy.id})")
            
            # Trigger the workflow
            alert_data = {
                "alert_id": f"alert-{alert_config.id}",
                "severity": ",".join(alert_levels),
                "agent_name": "system",
                "agent_ip": "0.0.0.0",
                "rule_description": alert_config.name
            }
            
            result = client.trigger_remediation_workflow(alert_data, policy)
            ...
            if result.get("success"):
                ...
                execution = RemediationExecution(
                    ...
                    request_payload=json.dumps(alert_data)
                )
                ...
                logger.info(f"✅ Remediation {policy.name} executed: {result.get('execution_id')}")
            else:
                logger.error(f"❌ Remediation {policy.name} failed: {result.get('error')}")
        
        db.session.commit()
    except Exception as e:
        logger.error(f"Error in trigger_remediation_workflows: {str(e)}")
            ...
            for alert_config in alert_configs:
                try:
                    ...
                    if is_high_critical:
                        ...
                        # Trigger n8n remediation workflows for high/critical alerts
                        try:
                            trigger_remediation_workflows(alert_config)
                        except Exception as e:
                            logger.error(f"Error triggering remediation workflows: {str(e)}")
