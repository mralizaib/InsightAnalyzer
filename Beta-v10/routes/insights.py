...
@insights_bp.route('/api/insights/results/<int:result_id>/voice-follow-up', methods=['POST'])
@login_required
def voice_follow_up(result_id):
    """Ask a follow-up question with voice support and multi-language"""
    try:
        ...
        result = AiInsightResult.query.filter_by(id=result_id).first()
        if not result:
            return jsonify({'error': 'Analysis result not found', 'success': False}), 404
        
        # Auto-detect language and get answer with multi-language support
        ...
        if result:
            result.add_follow_up(question, response.get('answer', response.get('summary', '')))
            db.session.commit()
        ...
        logger.error(f"Error in voice follow-up: {str(e)}")
...
def voice_qa():
    """
    Voice-powered Q&A for Wazuh and security questions.
    ...
    try:
        ...
        if include_context:
            try:
                ...
                if results and 'error' not in results:
                    context_data = results.get('results', [])
            except Exception as e:
                logger.warning(f"Could not fetch context data: {str(e)}")
        ...
        logger.info(f"Voice Q&A from {current_user.username}: {question[:50]}...")
        ...
        logger.error(f"Error in voice Q&A: {str(e)}")
        return jsonify({'error': str(e), 'success': False}), 500
