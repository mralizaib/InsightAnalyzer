class AIAssistant {
        ...
        try {
            ...
            if (result.error) {
                ...
                if (this.voiceModeEnabled && result.summary) {
                    const detectedLanguage = result.detected_language || 'English';
                    this.speakWithLanguage(result.summary, detectedLanguage);
                }
            } else {
    ...
    speakWithLanguage(text, language = 'English') {
        if (!text) return;
        
        // Try OpenAI TTS API first for better quality
        this.speakOpenAI(text, language);
    }
    
    async speakOpenAI(text, language) {
        try {
            ...
            if (result.success && result.audio_data) {
                // Play the audio from OpenAI TTS
                const audio = new Audio(`data:audio/mpeg;base64,${result.audio_data}`);
                audio.play().catch(err => {
                    console.log('OpenAI TTS playback failed, falling back to browser TTS:', err);
                    this.speak(text, language);  // Fall back to browser TTS
                });
            } else {
                // Fall back to browser TTS if API fails
                this.speak(text, language);
            }
        } catch (error) {
            console.log('OpenAI TTS error, using browser TTS:', error);
            this.speak(text, language);  // Fall back to browser TTS
        }
    }
    ...
    speak(text, language = 'English') {
        ...
        // Map language names to language codes (includes Punjabi, Sindhi, Urdu)
        const langMap = {
            'English': 'en-US', 'Spanish': 'es-ES', 'French': 'fr-FR',
            'German': 'de-DE', 'Italian': 'it-IT', 'Portuguese': 'pt-BR',
            'Russian': 'ru-RU', 'Japanese': 'ja-JP', 'Chinese': 'zh-CN',
            'Arabic': 'ar-SA', 'Hindi': 'hi-IN', 'Korean': 'ko-KR',
