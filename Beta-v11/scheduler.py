...
def store_alerts_in_database():
    ...
    try:
        with scheduler.app.app_context():
            ...
            # Fetch recent alerts (last 7 days for training)
            ...
            if results and 'error' not in results:
                ...
                for alert in alerts:
                    try:
                        ...
                        data_win = source.get('data', {}).get('win', {})
                        ...
                        stored_alert = StoredAlert(
                            ...
                            username=data_win.get('eventdata', {}).get('targetUserName'),
                            event_type=source.get('rule', {}).get('groups', [''])[0] if source.get('rule', {}).get('groups') else '',
                        ...
                        if stored_count % 50 == 0:
                            db.session.commit()
                            logger.info(f"Stored {stored_count} alerts for AI search training")
                    
                    except Exception as e:
                        logger.warning(f"Error storing alert {alert_id}: {str(e)}")
                        continue
                
                db.session.commit()
                logger.info(f"Successfully stored {stored_count} alerts for AI search training")
            else:
                logger.warning("Could not fetch alerts from OpenSearch")
