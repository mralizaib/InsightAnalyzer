        ...
        with scheduler.app.app_context():
            ...
            for report_config in report_configs:
                try:
                    ...
                    logger.info(f"Time comparison (Pakistan timezone) - Scheduled: {normalized_scheduled_time}, Current: {normalized_current_time}")
                    ...
                    if report_config.schedule == 'daily':
                        ...
                        logger.info(f"Daily report check - Match: {should_run}")
...
def update_scheduler_jobs():
    ...
    try:
        # Use the application context from the scheduler
        with scheduler.app.app_context():
            # Get alert check interval from system config or use default (5 minutes for faster response)
            interval_value = SystemConfig.get_value('alert_check_interval', '5')
            alert_check_interval = 5  # Default fallback - more frequent checks

            if interval_value:
                try:
                    alert_check_interval = int(interval_value)
                except (ValueError, TypeError):
                    logger.warning(f"Invalid alert_check_interval value: {interval_value}, using default 5")
            else:
                logger.warning("No alert_check_interval value found, using default 5")
    ...
    with app.app_context():
        ...
        try:
            if not SystemConfig.get_value('alert_check_interval'):
                SystemConfig.set_value('alert_check_interval', '300', 'Interval in seconds for checking alerts')
        ...
        # Add alert checking job - check every 5 minutes for new alerts
        scheduler.add_job(
            func=check_alerts,
            trigger="interval",
            minutes=5,
